...
public class AckMsg
{
	public AckMsg(int dstLink, int dstPort, int srcLink, int srcPort, short msgId) throws MalformedPacketException
	{	
		byte[] msgIdBuf = new byte[2];
		msgIdBuf[0] = (byte)(msgId >> 8);
		msgIdBuf[1] = (byte)(msgId & 0x00FF);
		byte[] contents = new byte[3];
		contents[0] = MailMessage.ACK;
	...
	public static boolean isAck(MailMessage msg)
	{
		boolean retval = false;
		if(msg.contents.length == 3)
		{
			if(msg.contents[0] == MailMessage.ACK)
			{
				retval = true;
			}
		}
